...
- **实施时已定稿(2026-06-07,见 `server/internal/nacos/REST-providers.md`)**:
  - REST:`/nacos/v3/admin/cs/config*`,**参数名 `groupName`(非 `group`)**;publish `POST cs/config`
    (form:`dataId`/`groupName`/`namespaceId`/`type=json`/`content`)是 **UPSERT**(再发布即覆盖,无 409
    → `RegisterProvider` 发布即注册);get `GET cs/config` 返回 `{code,message,data}` 信封,provider JSON
    在 `data.content` 是**转义字符串需二次 `Unmarshal`**;list `GET cs/config/list` 的 `pageItems` **不带
    content**(只元数据)→ `ListProviders` 逐个 get;delete `DELETE cs/config`(仅 dev 清理用)。
  - 版本:config 内容自带 `version` 字段为主,Nacos 配置历史(`cs/history`)作审计兜底;ref 钉具体历史
    版本属基础适配的非目标(get 取当前 published content)。
  - `namespaceId` 空 → Nacos 归一成 `public`;适配恒显式传 `shared`/workspace id。
  - **AI Registry 确认无 provider 资源类型**(`ai/provider|llm|model/list` 全 404)→ 定稿用配置中心。
- **实现期命名偏差**:派发钩子实际叫 `resolveAgentProviderRef`(spec §5 写的 `resolveAgentProvider` 与
  `agent.go` 既有的 runtime-provider 查询方法重名,Go 禁止同 receiver 重名,故加 `Ref` 后缀)。
