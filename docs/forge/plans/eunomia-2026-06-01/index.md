...
## 完成门禁（F4 DoD）
- [x] `forge_entropy_scan` 迁移 114 + sqlc（CRUD + `GetEntropyScanByAutopilot` + `ListOpenEntropyFindings`）
- [x] `checks.go` 迁 `internal/forge/checks` 子包，唯一调用点 `handler/forge_daemon.go` 改通，编译绿
- [x] `forgeentropy.ComposeBrief` 纯单测 2 个绿（含全段 / 省略空段）
- [x] `dispatchCreateIssue` 一处钩子注入合成 brief，`go build ./...` + vet 通过
- [x] `/api/forge/entropy-scans` CRUD + 代管 backing autopilot（POST 建/PATCH 改/DELETE 删同步）；
  并加固 scanner agent + project 的工作区归属校验（对齐原生 autopilot create）
- [x] UI `packages/views/forge-entropy/` + web 路由（三包 typecheck 绿；core 413 + views 954 单测绿）
- [x] **绕凭证集成（源码构建栈实测）**：API 建 scan → backing autopilot + schedule 触发已建、autopilot_id 回写；
  **手动触发 autopilot → run=issue_created → 断言 `issue.description` 含 `Entropy Scan` 标题 + `WHOLE-REPOSITORY`
  + 唯一 custom_focus 标记**（证明派发钩子合成 brief 并注入真 issue）；DELETE → backing autopilot 消失
