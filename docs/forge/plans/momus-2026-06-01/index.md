...
## 完成门禁（F3 DoD）
- [x] `forge_review_config` 迁移 113 + sqlc + `CreateForgeReviewTask` query（表在源码构建栈存在）
- [x] `ResolveReviewer` + `IsReviewTask` + `ShouldEnqueueReview` 单测（forgereview 包 3 测全绿）
- [x] `MaybeEnqueueReview`（service）建 review 任务（agent=reviewer、parent、work_dir、marker）——
  编译 + vet 通过；其决策门禁（gating + 防循环 + reviewer 解析）由 forgereview 单测覆盖
- [x] CompleteTask 一处钩子 + claim PriorWorkDir 钩子，`go build ./...` 通过
- [x] `/api/forge/review-config` GET/PUT + UI（三包 typecheck 绿；源码构建栈 PUT→GET 往返 OK）
- [x] **编排决策单测**（绕凭证）：`ShouldEnqueueReview` 断言 issue+workdir+非 review→建任务、
  review 任务自身→跳过（防循环）；`ResolveReviewer` 项目覆盖工作区。**活体串联**（真 completed
  coding task → 实际插 review 行 → reviewer 跑 git diff → 评论）待凭证，见下
