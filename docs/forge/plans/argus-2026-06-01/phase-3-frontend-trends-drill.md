## Phase 3 — 前端趋势图(recharts) + 钻取面板

**Goal:** 把 Phase 2 的快照 view 升级为完整面板:快照卡 + recharts 趋势图 + 混合钻取(卡可展开)。
**Depends-on:** Phase 1（趋势/钻取端点）、Phase 2（快照 view）　**Unblocks:** Phase 4
**Completion gate:** 三包 typecheck 绿。

> 钻取混合:fix PR 行链到外部 `pr_url`(真 URL);findings / gate-failures 在面内嵌轻量文本列表(兑底,
> 不构造 issue 内部 URL,避免脆弱)。recharts 3.8.0 已在 `packages/views` deps。

---

### Task 3.1: 完整 forge-health view（趋势 + 钻取）

**Files:**
- Modify: `packages/views/forge-health/forge-health-page.tsx`（整文件替换为下版）

- [ ] **Step 1: 替换为完整 view**

`packages/views/forge-health/forge-health-page.tsx`（整文件覆盖）：
```tsx
"use client";
...
Run: `cd D:\shulex_work\forge; corepack pnpm --filter "@multica/core" --filter "@multica/views" --filter "@multica/web" typecheck 2>&1 | Select-Object -Last 10`
Expected: 三包 Done。（若 recharts 类型报错,确认 `recharts` 在 `packages/views/package.json` deps——已核 3.8.0 在;React.ReactElement 用作 ResponsiveContainer child 类型若报错,改成 `import type { ReactElement } from "react"` + `children: ReactElement`。）

- [ ] **Step 3: Commit**

```bash
git add packages/views/forge-health/forge-health-page.tsx
git commit -m "feat(forge): F5 health trends (recharts) + drill-down panels"
```

---

## Phase 3 完成检查
- [ ] 3 条 recharts 趋势图(熵发现 line / 门禁通过率 line / 修复 PR bar)
- [ ] 混合钻取:findings/gate-failures 内嵌列表 + fix PR 外链,三包 typecheck 绿
